...
- **Build the engine module first, with its own unit tests against the rule set.** Get the engine green against its own tests before wiring `save()` / `delete()` through it. The acceptance criterion is "existing `RelationshipDeletionSemanticsTest` continues to pass without test-body changes when wired through the engine"; treat this as the post-migration gate.
